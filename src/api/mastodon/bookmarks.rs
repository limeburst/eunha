...
pub async fn get_bookmarks(
    ...
    // Fetch (sort_id, status_id) pairs ordered by bookmark sort_id so that
    // pagination reflects the time the bookmark was created, not the status age.
    struct BRow { sort_id: i64, status_id: i64 }
    ...
    // Fetch the actual status rows, then re-sort into bookmark order.
    ...
    // Rebuild in bookmark sort order (drop any status that was deleted between queries).
